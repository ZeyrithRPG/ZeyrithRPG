...
def rolar_materiais(session, texto_materiais_dropados, bonus_drop_pct: float = 0.0):
    """Retorna lista de (nome_material, quantidade) que efetivamente dropou.
    Aplica bônus de drop de proficiência (+1% a cada 3 níveis) somado à chance base, com teto em 100%.
    """
    from db.models import Material

    materiais = _parse_materiais(texto_materiais_dropados)
    dropados = []
    bonus = max(0.0, float(bonus_drop_pct or 0.0)) / 100.0
    for nome, anotacao in materiais:
        ...
        chance_final = min(1.0, chance + bonus)
        if random.random() <= chance_final:
            dropados.append((nome, 1))
    return dropados


def resolver_loot(session, player, monstro, curva_mestra_jogador, bonus_drop_pct: float = 0.0):
    ...
    ouro_base = calcular_ouro(monstro.hp, monstro.papel)
    mult_ouro = 1.0
    if player:
        if getattr(player, "titulo_ativo", None) == "Bolso Fundo":
            mult_ouro += 0.05
        try:
            mutacoes = getattr(player, "mutacoes", None)
            if mutacoes and any(getattr(m, "mutacao_id", 0) == 19 or "Aura de Decomposição" in (getattr(m, "nome", "") or "") for m in mutacoes):
                mult_ouro += 0.05
        except Exception:
            pass
    ouro = max(1, round(ouro_base * mult_ouro))
    materiais = rolar_materiais(session, monstro.materiais_dropados, bonus_drop_pct=bonus_drop_pct)
...
CHANCE_DROP_EQUIPAMENTO = {"Comum": 0.12, "Elite": 0.45, "Boss": 1.0, "Cosmico": 1.0}


def sortear_equipamento_loot(session, monstro):
    """
    Retorna None ou um dict {"tipo_item": "arma"|"armadura", "item_ref_id": int,
    "nome_item": str, "danificado": True} pronto pra virar PlayerInventario.
    Ao Nv do monstro, mapeia pro Tier e sorteia entre Arma e Armadura DAQUELE Tier,
    com 50/50 de chance entre as duas categorias.
    """
    import random
    from db.models import Arma, Armadura, Tier

    chance = CHANCE_DROP_EQUIPAMENTO.get(monstro.papel, 0.12)
    if random.random() > chance:
        return None

    tier = (
        session.query(Tier)
        .filter(Tier.nivel_min <= monstro.nivel, Tier.nivel_max >= monstro.nivel)
        .first()
    )
    if not tier:
        return None
    ...
    if random.random() < 0.5:
        candidatos = session.query(Arma).filter_by(tier=tier.nome).all()
        tipo_item = "arma"
    else:
        candidatos = session.query(Armadura).filter_by(tier=tier.nome).all()
        tipo_item = "armadura"
    ...
    return {
        "tipo_item": tipo_item,
        "item_ref_id": item.id,
        "nome_item": f"{item.variacao} (Danificado)",
        "danificado": True,
    }
