...
PLANILHA = os.path.join(RAIZ, "balanceamento_rpg_V1.0.xlsx")
...
def recalcular():
    ...
    subprocess.run(
        [soffice_bin, "--headless", "--calc", "--convert-to", "xlsx", "--outdir", tmpdir, PLANILHA],
...
def extrair_curva_mestra(wb):
    ...
    for r in range(20, 95):
        ...
        out.append({
            ...
            "defesa_esperada": v(ws, r, 4), "atq_bonus": v(ws, r, 5), "mana": v(ws, r, 13),
...
def extrair_classes(wb):
    ...
    for r in range(3, 11):
        ...
        out.append({
            ...
            "vigor_inicial": v(ws, r, 14),
            "hp_mult": v(ws, r, 17),
            "def_nativa": v(ws, r, 18),
            "crit_base": v(ws, r, 19),
            "mana_mult": v(ws, r, 20),
            "papel": v(ws, r, 3),
            "emoji": v(ws, r, 21),
...
def extrair_habilidades_ativas(wb):
    ws = wb["Habilidades_Ativas"]
    out = []
    for r in range(2, 10):
        classe_id = v(ws, r, 1)
        if not classe_id:
        ...
        out.append({
            ...
            "classe_nome": v(ws, r, 2),
            "habilidade_nome": v(ws, r, 3),
            "tipo_acao": v(ws, r, 4),
            "recurso_tipo": v(ws, r, 5),
            "custo_recurso": int(v(ws, r, 6)),
            "dano_pct_rank1": float(v(ws, r, 7)),
            "dano_pct_rank2": float(v(ws, r, 8)),
            "dano_pct_rank3": float(v(ws, r, 9)),
            "dano_pct_rank4": float(v(ws, r, 10)),
            "dano_pct_rank5": float(v(ws, r, 11)),
            "efeito_secundario_tipo": v(ws, r, 12),
            "efeito_secundario_rank1": float(v(ws, r, 13)),
            "efeito_secundario_rank2": float(v(ws, r, 14)),
            "efeito_secundario_rank3": float(v(ws, r, 15)),
            "efeito_secundario_rank4": float(v(ws, r, 16)),
            "efeito_secundario_rank5": float(v(ws, r, 17)),
            "custo_upgrade_ouro_rank1": int(v(ws, r, 18)),
            "custo_upgrade_ouro_rank2": int(v(ws, r, 19)),
            "custo_upgrade_ouro_rank3": int(v(ws, r, 20)),
            "custo_upgrade_ouro_rank4": int(v(ws, r, 21)),
            "custo_upgrade_ouro_rank5": int(v(ws, r, 22)),
            "descricao": v(ws, r, 23),
        })
    return out
...
def _preco_compra_venda(preco_venda_base, mod_variacao=None):
    if not isinstance(preco_venda_base, (int, float)):
        return None, None
    compra = round(preco_venda_base * 1.5)
    if isinstance(mod_variacao, (int, float)) and mod_variacao > 0 and abs(mod_variacao - 1.05) > 0.001:
        # Quando mod_variacao difere do padrão (1.05), calibra o preço de compra proporcionalmente
        compra = round(compra * (mod_variacao / 1.05))
...
def extrair_armas(wb):
    ...
    for r in range(15, 325):
        ...
        if not variacao or not tier or tier == "Tier":
            continue
        mod_var = v(ws, r, 11)
        compra, venda = _preco_compra_venda(v(ws, r, 12), mod_var)
        ef_chance = v(ws, r, 14) or 0.0
        ef_valor = v(ws, r, 15) or 0.0
        out.append({
            "tier": tier, "tipo": v(ws, r, 2), "variacao": variacao,
            "efeito_especial": v(ws, r, 4),
            "efeito_chance_pct": float(ef_chance),
            "efeito_valor": float(ef_valor),
            "lore": v(ws, r, 5),
            "mod_variacao": mod_var, "dano_comum": v(ws, r, 9),
...
def extrair_receitas(wb):
    ...
    for r in range(5, 600):
        ...
        if categoria == "Arma/Armadura":
            mat1 = v(ws, r, 3)
            mat2 = v(ws, r, 4)
            efeito = v(ws, r, 10)
        else:
            # Para Ferramenta de Coleta, Comida/Bebida, Acessório:
            # Col 3 contém a lista de materiais (ex: '2x Sucata de Ferro' ou '1x A + 1x B')
            # Col 4 contém o efeito/passiva (ex: '+1 em testes físicos')
            materiais_raw = v(ws, r, 3) or ""
            parts = [p.strip() for p in str(materiais_raw).split("+") if p.strip()]
            mat1 = parts[0] if len(parts) > 0 else None
            mat2 = " + ".join(parts[1:]) if len(parts) > 1 else None
            efeito = v(ws, r, 4) or v(ws, r, 10)

        custo_nv2 = v(ws, r, 7)
        out.append({
            "tier": tier, "tipo_slot": tipo_slot,
            "material_base_1": mat1, "material_base_2": mat2,
            ...
            "artesao_mestre": v(ws, r, 8), "categoria": categoria,
            "efeito": efeito,
        })
    return out
...
def extrair_titulos(wb):
    ...
    for r in range(5, 40):
        ...
        out.append({
            ...
            "condicao": v(ws, r, 2),
            "bonus": v(ws, r, 3),
            "condicao_tipo": v(ws, r, 4),
            "condicao_valor": v(ws, r, 5),
            "bonus_tipo": v(ws, r, 6),
            "bonus_valor": v(ws, r, 7),
        })
    return out
...
def extrair_consumiveis(wb):
    ws = wb["Consumiveis"]
    out = []
    for r in range(14, 70):
        tier = v(ws, r, 1)
        nome = v(ws, r, 2)
        if not tier or not nome or nome == "(Retirado)":
            continue
        efeito = v(ws, r, 3)
        preco = v(ws, r, 4)
        m = re.search(r"\d+", str(efeito)) if efeito else None
        qtd = int(m.group()) if m else None
        tipo = "cura" if "Cura" in nome else ("mana" if "Mana" in nome else "antidoto")
        out.append({
            "tier": tier, "nome": nome, "tipo": tipo,
            "efeito_descricao": efeito, "quantidade_recuperada": qtd,
            "preco_compra": preco,
        })
    return out


def extrair_debuffs_derrota(wb):
    if "Debuffs de Derrota" not in wb.sheetnames:
        return []
    ws = wb["Debuffs de Derrota"]
    out = []
    for r in range(2, 7):
        nome = v(ws, r, 1)
        ...
        out.append({
            "nome": nome,
            "chance_pct": float(v(ws, r, 2) or 0),
            "efeito": v(ws, r, 3),
            "cura": v(ws, r, 4),
        })
    return out
...
def main():
    ...
    dados = {
        ...
        "habilidades_ativas": extrair_habilidades_ativas(wb),
        "consumiveis": extrair_consumiveis(wb),
        ...
        "debuffs_derrota": extrair_debuffs_derrota(wb),
    }
