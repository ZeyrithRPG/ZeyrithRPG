...
class Classe(Base):
    ...
    # --- Ciclo 3: campos estruturados de progressão direta por classe ---
    hp_mult = Column(Float, default=1.0)
    def_nativa = Column(Integer, default=0)
    crit_base = Column(Float, default=0.0)
    vigor_inicial = Column(Integer, default=60)
    mana_mult = Column(Float, default=1.0)
    papel = Column(String)
    emoji = Column(String)


class HabilidadeAtiva(Base):
    __tablename__ = "ref_habilidades_ativas"
    id = Column(Integer, primary_key=True)
    classe_id = Column(Integer, ForeignKey("ref_classes.id"), nullable=False, unique=True)
    classe_nome = Column(String, nullable=False)
    habilidade_nome = Column(String, nullable=False)
    tipo_acao = Column(String, nullable=False)  # "Física" ou "Mágica"
    recurso_tipo = Column(String, nullable=False)  # "Vigor" ou "Mana"
    custo_recurso = Column(Integer, nullable=False)
    dano_pct_rank1 = Column(Float, nullable=False)
    dano_pct_rank2 = Column(Float, nullable=False)
    dano_pct_rank3 = Column(Float, nullable=False)
    dano_pct_rank4 = Column(Float, nullable=False)
    dano_pct_rank5 = Column(Float, nullable=False)
    efeito_secundario_tipo = Column(String, nullable=False)
    efeito_secundario_rank1 = Column(Float, nullable=False)
    efeito_secundario_rank2 = Column(Float, nullable=False)
    efeito_secundario_rank3 = Column(Float, nullable=False)
    efeito_secundario_rank4 = Column(Float, nullable=False)
    efeito_secundario_rank5 = Column(Float, nullable=False)
    custo_upgrade_ouro_rank1 = Column(Integer, default=0)
    custo_upgrade_ouro_rank2 = Column(Integer, default=60)
    custo_upgrade_ouro_rank3 = Column(Integer, default=180)
    custo_upgrade_ouro_rank4 = Column(Integer, default=420)
    custo_upgrade_ouro_rank5 = Column(Integer, default=850)
...
class Arma(Base):
    ...
    efeito_chance_pct = Column(Float, default=0.0)
    efeito_valor = Column(Float, default=0.0)
    lore = Column(Text)
...
class Consumivel(Base):
    __tablename__ = "ref_consumiveis"
    id = Column(Integer, primary_key=True)
    tier = Column(String)
    nome = Column(String, nullable=False)
    tipo = Column(String)          # "cura", "mana", "antidoto"
    efeito_descricao = Column(String)   # string já pronta tipo "Recupera 30 HP"
    quantidade_recuperada = Column(Integer)  # valor numérico extraído do efeito_descricao
    preco_compra = Column(Integer)
...
class Player(Base):
    ...
    habilidade_ativa_nivel = Column(Integer, default=1)
    ...
    slots_mochila_max = Column(Integer, default=20)
    ultimo_reset_missoes = Column(String, nullable=True)
    rerolls_restantes = Column(Integer, default=2)
    titulo_ativo = Column(String, nullable=True)
    debuff_ativo = Column(String, nullable=True)
    combates_vencidos_qtd = Column(Integer, default=0)
    fugas_com_sucesso_qtd = Column(Integer, default=0)
    reparos_feitos_qtd = Column(Integer, default=0)
    ouro_acumulado_pico = Column(Integer, default=0)
    ...
    receitas = relationship("PlayerReceita", back_populates="player")
    mutacoes = relationship("PlayerMutacao", back_populates="player")
    titulos = relationship("PlayerTitulo", back_populates="player")
    talentos = relationship("PlayerTalento", back_populates="player")
...
class PlayerInventario(Base):
    ...
    danificado = Column(Boolean, default=False)
...
class PlayerQuest(Base):
    ...
    progresso_meta = Column(Integer, default=1)
    tipo_objetivo = Column(String, nullable=True)  # 'abate' ou 'coleta'
    alvo_id_ou_nome = Column(String, nullable=True)
    local_alvo = Column(String, nullable=True)

    player = relationship("Player")
    quest = relationship("Missao")


class PlayerReceita(Base):
    __tablename__ = "player_receitas"
    id = Column(Integer, primary_key=True)
    player_id = Column(Integer, ForeignKey("players.id"))
    receita_id = Column(Integer, ForeignKey("ref_receitas.id"))
    desbloqueada = Column(Boolean, default=False)

    player = relationship("Player", back_populates="receitas")
    receita = relationship("Receita")
    __table_args__ = (UniqueConstraint("player_id", "receita_id"),)


class PlayerMutacao(Base):
    __tablename__ = "player_mutacoes"
    id = Column(Integer, primary_key=True)
    player_id = Column(Integer, ForeignKey("players.id"))
    mutacao_id = Column(Integer, nullable=False)  # 1-20
    nome = Column(String, nullable=False)
    categoria = Column(String, nullable=True)
    deformacao = Column(Text, nullable=True)
    bonus_mecanico = Column(Text, nullable=True)
    penalidade = Column(Text, nullable=True)
    lore = Column(Text, nullable=True)
    data_aquisicao = Column(DateTime, default=datetime.utcnow)

    player = relationship("Player", back_populates="mutacoes")
...
class PlayerTalento(Base):
    ...
    player = relationship("Player", back_populates="talentos")
    talento = relationship("TalentoClasse")
    __table_args__ = (UniqueConstraint("player_id", "talento_id"),)
...
class DebuffDerrota(Base):
    __tablename__ = "ref_debuffs_derrota"
    id = Column(Integer, primary_key=True)
    nome = Column(String, unique=True, nullable=False)
    chance_pct = Column(Float, nullable=False)
    efeito = Column(Text, nullable=False)
    cura = Column(Text, nullable=False)
...
class Titulo(Base):
    ...
    bonus = Column(Text)
    condicao_tipo = Column(String, nullable=True)
    condicao_valor = Column(String, nullable=True)
    bonus_tipo = Column(String, nullable=True)
    bonus_valor = Column(Float, nullable=True)
...
class PlayerTitulo(Base):
    ...
    player = relationship("Player", back_populates="titulos")
